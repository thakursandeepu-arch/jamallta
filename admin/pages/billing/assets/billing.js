...
function getActiveClientNames() {
  ...
  allClients.forEach((client) => {
    const name = cleanName(client.studioName || client.customerName || client.name);
...
function getJobTotalAmount(job) {
  const itemsTotal = Array.isArray(job?.itemsAdded)
    ? job.itemsAdded.reduce((a, i) => a + Number(i.rowTotal || 0), 0)
    : 0;
  return Number(job?.totalAmount || 0) || itemsTotal;
}

function getJobPendingAmount(job) {
  const totalAmount = getJobTotalAmount(job);
  const storedPending = Number(job?.pendingAmount);
  if (!Number.isNaN(storedPending)) return Math.max(storedPending, 0);
  const paidAmount = Number(job?.paidAmount || 0);
  return Math.max(totalAmount - paidAmount, 0);
}

function getClientPendingBalance(client) {
  const balance = Number(client?.balance);
  if (!Number.isNaN(balance)) return Math.max(balance, 0);
  const outstanding = Number(client?.outstandingBalance);
  if (!Number.isNaN(outstanding)) return Math.max(outstanding, 0);
  return 0;
}
...
function startListeners() {
  ...
  onSnapshot(collection(db, "customers"), snap => {
    ...
    snap.forEach(d => {
      const c = d.data();
      if (!c || c.deleteData) return;
      allClients.push({ id: d.id, ...c });
      const primaryName = cleanName(c.studioName || c.customerName || c.name);
      if (primaryName) allStudios.push(primaryName);
    });
    allStudios = getActiveClientNames();
    updateStats();
    renderClientsList();
  }, (err) => {
...
function updateStats() {
  ...
  let revenue = 0;
  allJobs.forEach(j => {
    revenue += getJobTotalAmount(j);
  });

  const pending = allClients.length
    ? allClients.reduce((a, c) => a + getClientPendingBalance(c), 0)
    : allJobs.reduce((a, j) => a + getJobPendingAmount(j), 0);

  totalRevenueEl.textContent = "₹" + money(revenue);
  pendingBalanceEl.textContent = "₹" + money(pending);
  totalClientsEl.textContent = getActiveClientNames().length;
...
function renderClientsList() {
  ...
  const clients = getActiveClientNames();
  ...
  const clientKeyToName = new Map(clients.map(name => [normalizeName(name), name]));
  allJobs.forEach(j => {
    const rawName = cleanName(j.studioName || j.customerName || "");
    const clientName = clientKeyToName.get(normalizeName(rawName));
...
window.saveNewJob = async () => {
  ...
  if (!allStudios.some(name => normalizeName(name) === normalizeName(studioName))) {
