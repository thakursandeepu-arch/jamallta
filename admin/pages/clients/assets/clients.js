...
const callSendCustomerEmail = async (payload) => {
  const fn = httpsCallable(functions, "sendCustomerEmail");
  const res = await fn(payload || {});
  return res?.data || {};
};
...
function main() {
  ...
  const btnEmailCustomer = document.getElementById("btnEmailCustomer");
  ...
  function getCustomerEmail(customer = null) {
    return (customer?.email || currentCustomerEmail || emailInput?.value || "").toString().trim();
  }

  async function sendCustomerEmailNow(customer = null, button = null) {
    const email = getCustomerEmail(customer);
    if (!email) {
      showToast("Customer Gmail/email not saved", "warning");
      return;
    }
    const studioName = customer?.studioName || studioNameInput?.value || currentStudio || "";
    const balance = customer?.balance ?? Number(liveBalanceSpan?.textContent || 0);
    ...
    try {
      await callSendCustomerEmail({ email, studioName, balance });
      showToast("Email sent to customer");
    } catch (error) {
      console.error("Customer email failed:", error);
      showToast(error?.message || "Email send failed", "error");
    } finally {
      if (button) {
        button.disabled = false;
        button.textContent = oldText || "Email";
      }
    }
  }
  ...
  async function openProfile(studioName){
    ...
    if(!custSnap.empty && custSnap.docs[0]){
      ...
      if (btnEmailCustomer) btnEmailCustomer.disabled = !currentCustomerEmail.trim();
      ...
      if (btnEmailCustomer) btnEmailCustomer.disabled = true;
    }
  ...
  if (btnEmailCustomer) {
    btnEmailCustomer.textContent = "Send Email";
    btnEmailCustomer.addEventListener("click", () => sendCustomerEmailNow(null, btnEmailCustomer));
  }
  ...
  if(btnEditCustomer) btnEditCustomer.addEventListener("click", async ()=>{
    ...
    if(!editing){
      ...
      if (btnEmailCustomer) btnEmailCustomer.disabled = !getCustomerEmail().trim();
